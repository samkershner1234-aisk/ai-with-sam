...
export default function HowItWorks() {
  return (
    ...
    <p style={{fontSize:"13px",fontWeight:600,letterSpacing:"0.1em",color:" #F97316",textTransform:"uppercase",textAlign:"center",marginBottom:"16px"}}>The Process</p>p>
    <h2 style={{fontSize:"clamp(28px,4vw,40px)",fontWeight:800,color:" #1E293B",textAlign:"center",marginBottom:"48px"}}>Here's Exactly What Happens</h2>h2>
    <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:"24px",marginBottom:"48px"}} className="sg">
      ...
      <div style={{fontSize:"48px",fontWeight:800,color:" #F97316",marginBottom:"16px",lineHeight:1}}>{s.num}</div>div>
      <h3 style={{fontWeight:700,color:" #1E293B",fontSize:"20px",marginBottom:"12px"}}>{s.title}</h3>h3>
      <p style={{color:" #64748B",fontSize:"15px",lineHeight:1.7,marginBottom:"16px"}}>{s.body}</p>p>
    ...
    <p style={{color:" #64748B",fontSize:"13px",marginTop:"10px"}}>Free call. No credit card. No commitment.</p>p>
    </div>div>
    </div>div>
    <style>{`@media(max-width:768px){.sg{grid-template-columns:1fr!important}}`}</style>style>
    </section>section>
    ...
    <p style={{fontSize:"13px",fontWeight:600,letterSpacing:"0.1em",color:" #F97316",textTransform:"uppercase",marginBottom:"16px"}}>Clarity</p>p>
    <h2 style={{fontSize:"clamp(24px,3vw,36px)",fontWeight:800,color:" #1E293B",marginBottom:"20px"}}>What This Is NOT</h2>h2>
    <p style={{color:" #475569",fontSize:"17px",lineHeight:1.8,margin:0}}>This is NOT a course. NOT a tutorial series. NOT a workshop where you take notes and try things at home later. This is one hour, one problem, one working system, built live. If you want theory, there are thousands of YouTube channels for that. If you want something that works by tonight, this is it.</p>p>
    </div>div>
    </section>section>
    </>>
    );
